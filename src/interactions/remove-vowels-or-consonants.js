...
const removeVowelsOrConsonants = () => {
  ...
  if (select === 'vowels') {
    let change = removeCharacters(userOption, charactersToRemove = 'AEIOUaeiou');
  }
  //  remove all consonants (lower and upper case) if the user selected "consonants"
  if (select === 'consonants') {
    change = removeCharacters(userOption, charactersToRemove = 'bcdfghklmnpqrstvwxyzBCDFGHKLMNPQRSTVWXYZ');
  }
  // 4. render a final before/after message with favoriteWord and the new string from ^ up there
  let finalMessage = beforeAndAfter(userOption, change, description = 'all the vowels or all the consonants removed')
